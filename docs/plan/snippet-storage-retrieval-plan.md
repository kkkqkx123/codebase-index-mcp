...
### 第一阶段：设计和实现snippet提取逻辑（预计2周）

**第1周：核心功能开发**
- **任务1.1**：扩展TreeSitterService以识别snippets（3天）
...
**第3周：存储服务扩展**
- **任务2.1**：修改VectorStorageService以支持snippet存储（3天）
...
- **任务2.2**：集成Qdrant存储机制（2天）
  - 配置Qdrant集合以支持snippet存储
  - 实现chunkType字段过滤和索引

**第4周：检索服务扩展**
- **任务2.3**：扩展SemanticSearchService以支持snippet检索（3天）
  - 添加snippet专用检索方法
  - 实现基于chunkType的过滤器
  - 开发snippet相关性排序算法
  - 实现基于主题的聚类和层次化存储
...
### 第三阶段：集成到现有索引流程（预计1.5周）

**第5周：索引流程集成**
- **任务3.1**：在索引服务中集成snippet处理（3天）
