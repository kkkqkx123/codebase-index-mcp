...
export class NebulaSessionMonitor {
    ...
    this.monitoringInterval = setInterval(async () => {
      try {
        ...
        this.errorHandler.handleError(
          new Error(`Session monitoring failed: ${error instanceof Error ? error.message : String(error)}`),
          { component: 'NebulaSessionMonitor', operation: 'monitoringCycle' }
        );
      }
    }, intervalMs);

    this.logger.info(`Started NebulaGraph session monitoring with ${intervalMinutes} minute interval`);
    
    // 立即执行一次监控
    this.performMonitoringCycle().catch(error => {
      this.errorHandler.handleError(error, { component: 'NebulaSessionMonitor', operation: 'initialMonitoring' });
    });
  }
  ...
  private async performMonitoringCycle(): Promise<void> {
    ...
    try {
      ...
      // 4. 清理空闲会话（每2小时清理一次）
      const now = new Date();
      if (now.getMinutes() % 120 === 0) { // 每2小时清理一次
      ...
      this.errorHandler.handleError(
        new Error(`Monitoring cycle failed: ${error instanceof Error ? error.message : String(error)}`),
  ...
  private logDetailedSessionInfo(sessionsInfo: any): void {
    if (sessionsInfo.allSessions && sessionsInfo.allSessions.length > 0) {
      this.logger.debug(`当前活跃会话详情: ${JSON.stringify(sessionsInfo.allSessions.slice(0, 5))}`);
    }
