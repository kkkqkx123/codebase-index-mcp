...
export class ConfigFactory implements ConfigFactory {
  ...
  private getEnvironmentConfig(): EnvironmentConfig {
    ...
    return {
      nodeEnv: (this.configService.get('nodeEnv') || process.env.NODE_ENV || 'development') as 'development' | 'production' | 'test',
      port: this.configService.get('port') || parseInt(process.env.PORT || '3000'),
      host: 'localhost',
      logLevel: (this.configService.get('logging')?.level || process.env.LOG_LEVEL || 'info') as 'debug' | 'error' | 'warn' | 'info',
      debug: false,
      corsEnabled: true,
  ...
  private getServicesConfig(): ServiceConfig {
    ...
    return {
      ...
      embedding: {
        provider: (config.embedding?.provider || process.env.EMBEDDING_PROVIDER || 'openai') as 'openai' | 'ollama' | 'gemini' | 'mistral' | 'local',
        model: config.embedding?.openai?.model || process.env.OPENAI_MODEL || 'text-embedding-ada-002',
      ...
      processing: {
        batchProcessing: {
          ...
          adaptiveBatching: {
            ...
            performanceThreshold: config.batchProcessing?.adaptiveBatching?.performanceThreshold || 1000,
  ...
  private initializeConfig(): void {
    try {
      ...
      this.errorHandler.handleError(
        new Error(`Failed to initialize configuration factory: ${error instanceof Error ? error.message : String(error)}`),
