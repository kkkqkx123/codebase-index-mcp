...
export class ProjectMetricsExporter {
  ...
  private async collectQdrantMetrics(projectId: string): Promise<void> {
    try {
      ...
      console.log(
        `qdrant_collection_size_bytes{collection="${collectionName}", project_id="${projectId}"} ${collectionInfo.pointsCount}`
      );
      console.log(
        `qdrant_collection_vectors_count{collection="${collectionName}", project_id="${projectId}"} ${collectionInfo.pointsCount}`
      );
