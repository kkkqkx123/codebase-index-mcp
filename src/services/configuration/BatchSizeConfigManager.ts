...
export class BatchSizeConfigManager {
  ...
  private calculateOptimalBatchSize(
    ...
    const memoryPressureScore =
      1 - Math.min(1, memoryUsage.heapUsed / memoryUsage.heapTotal / (config.memoryLimit / 100));
    ...
    optimalBatchSize = Math.max(
      config.minBatchSize,
      Math.min(optimalBatchSize, config.maxBatchSize)
    );
  ...
  recordOperationMetrics(
    ...
    stats.averageThroughput =
      alpha * (metrics.throughput || 0) + (1 - alpha) * stats.averageThroughput;
    stats.averageErrorRate =
      alpha * (metrics.errorRate || 0) + (1 - alpha) * stats.averageErrorRate;
    stats.averageMemoryUsage =
      alpha * ((metrics.memoryUsage.end / process.memoryUsage().heapTotal) * 100) +
    ...
    for (const [key, stats] of this.stats.entries()) {
      ...
      if (Math.abs(optimalBatchSize - stats.currentBatchSize) > stats.currentBatchSize * 0.1) {
        ...
        this.logger.info('Batch size adjusted', {
          ...
          oldBatchSize:
            optimalBatchSize !== stats.currentBatchSize ? stats.currentBatchSize : 'unknown',
