...
export class SpringBootRule extends AbstractSnippetRule {
  ...
  private extractSpringBootMetadata(
    node: Parser.SyntaxNode,
    content: string,
    sourceCode: string
  ) {
  ...
  private extractEndpoints(controllerContent: string) {
    ...
    const mappingPatterns = [
      { method: 'GET', pattern: /@(GetMapping|RequestMapping\s*\(\s*method\s*=\s*RequestMethod\.GET\s*\))\s*\b\s*public\s+\w+\s+(\w+)\s*\(([^)]*)\)/g },
      { method: 'POST', pattern: /@(PostMapping|RequestMapping\s*\(\s*method\s*=\s*RequestMethod\.POST\s*\))\s*\b\s*public\s+\w+\s+(\w+)\s*\(([^)]*)\)/g },
      { method: 'PUT', pattern: /@(PutMapping|RequestMapping\s*\(\s*method\s*=\s*RequestMethod\.PUT\s*\))\s*\b\s*public\s+\w+\s+(\w+)\s*\(([^)]*)\)/g },
      { method: 'DELETE', pattern: /@(DeleteMapping|RequestMapping\s*\(\s*method\s*=\s*RequestMethod\.DELETE\s*\))\s*\b\s*public\s+\w+\s+(\w+)\s*\(([^)]*)\)/g }
    ];
  ...
  private extractMethodParameters(parametersStr: string): string[] {
    return parametersStr.split(',').map(param => param.trim()).filter(param => param.length > 0);
  }
  ...
  private extractMappingPath(methodContent: string): string {
    const pathPattern = /@(GetMapping|PostMapping|PutMapping|DeleteMapping|RequestMapping)\s*\(\s*["']([^"']+)["']/;
  ...
  private detectCircularDependencies(content: string): boolean {
    ...
    const beanCount = (content.match(/@(Component|Service|Repository|Controller|RestController)/g) || []).length;
  ...
  private extractRepositories(content: string): string[] {
    ...
    const repositoryPattern = /@(Repository|JpaRepository|CrudRepository|PagingAndSortingRepository)\s*\b\s*public\s+interface\s+(\w+)/g;
    let match;
  ...
  private extractPerformanceInfo(content: string) {
    ...
    const cachingEnabled = content.includes('@Cacheable') || content.includes('@CacheEvict') || content.includes('@CachePut');
    const connectionPooling = content.includes('@EnableTransactionManagement') || content.includes('HikariCP');
  ...
  private calculateSpringBootComplexity(content: string): number {
    ...
    complexity += (content.match(/@(GetMapping|PostMapping|PutMapping|DeleteMapping)/g) || []).length * 2;
