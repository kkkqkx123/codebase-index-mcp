...
export class HybridSearchService {
  ...
  async searchWithFeedback(params: HybridSearchParams, feedback: {
    ...
    try {
      ...
      const weightAdjustment = this.adjustWeightsFromFeedback(feedback);
      ...
      this.errorHandler.handleError(
        new Error(`Hybrid search with feedback failed: ${error instanceof Error ? error.message : String(error)}`),
  ...
  async getSearchExplanation(result: HybridSearchResult, query: string): Promise<{
    resultId: string;
    query: string;
    finalScore: number;
    scoreBreakdown: {
    ...
    try {
      ...
      const explanation = {
        ...
        scoreBreakdown: {
          semantic: {
            ...
            contribution: this.calculateContribution(scores.semanticScore || 0, scores.combinedScore),
            factors: this.getSemanticFactors(result, query)
          },
          keyword: {
            score: scores.keywordScore || 0,
            contribution: this.calculateContribution(scores.keywordScore || 0, scores.combinedScore),
          ...
          structural: {
            ...
            contribution: this.calculateContribution(scores.structuralScore || 0, scores.combinedScore),
      ...
      this.errorHandler.handleError(
        new Error(`Search explanation generation failed: ${error instanceof Error ? error.message : String(error)}`),
  ...
  private async executeParallelSearches(params: HybridSearchParams, strategies: string[]): Promise<{
  ...
  private async executeSemanticSearch(params: HybridSearchParams): Promise<{ results: any[]; time: number }> {
    const startTime = Date.now();
  ...
  private async executeKeywordSearch(params: HybridSearchParams): Promise<{ results: any[]; time: number }> {
    const startTime = Date.now();
  ...
  private async executeFuzzySearch(params: HybridSearchParams): Promise<{ results: any[]; time: number }> {
    const startTime = Date.now();
  ...
  private async executeStructuralSearch(params: HybridSearchParams): Promise<{ results: any[]; time: number }> {
    const startTime = Date.now();
  ...
  private async fuseResults(searchResults: any, params: HybridSearchParams): Promise<HybridSearchResult[]> {
  ...
  private rankAndFilterResults(results: HybridSearchResult[], params: HybridSearchParams): HybridSearchResult[] {
  ...
  private generateMatchHighlights(result: HybridSearchResult, query: string): Array<{
    type: 'keyword' | 'semantic' | 'fuzzy' | 'structural';
    text: string;
    score: number;
  }> {
  ...
  private generateMatchDetails(result: HybridSearchResult, queryTerms: string[]): Array<{
    type: string;
    matchedText: string;
    relevance: number;
    context: string;
